...
function queueTime(customers, n) {
    // Create an array to represent the checkout tills
    const tills = Array(n).fill(0)

    // Iterate through the customers
    for(const time of customers){
      // Find the till with the minimum total time
      const minTillIndex = tills.indexOf(Math.min(...tills))
      // Assign the current customer's time to the selected till
      tills[minTillIndex] += time
    }
    // Return the maximum total time among all tills
